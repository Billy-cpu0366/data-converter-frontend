...
const StableApp: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8001/convert-stable', formData, {
      ...
      if (response.data.success) {
        ...
        setState(prev => ({ ...prev, error: response.data.error, loading: false }));
      }
    } catch (err: any) {
      setState(prev => ({ 
        ...prev, 
        error: err.response?.data?.error || '转换失败', 
        loading: false 
      }));
    }
  };
  ...
  const handleGeneratePractice = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8001/generate-stable-practice', {
